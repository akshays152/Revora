...
class TelemetrySnapshot(BaseModel):
    ...
    lap_id: str = "lap-42"
    telemetry_source: str = "DEMO"
    lap_number: int = Field(default=42, ge=1)
    lap_time_seconds: float | None = Field(default=103.821, gt=0)
    baseline_lap_time_seconds: float | None = Field(default=102.000, gt=0)
    # Convention: positive lap_delta (+0.8s) means SLOWER than reference pace
    lap_delta: float | None = Field(default=1.821)
    delta_was_corrected: bool = False
    sector_deltas: list[float] = Field(
        default_factory=lambda: [0.2, 0.9, 0.721])
