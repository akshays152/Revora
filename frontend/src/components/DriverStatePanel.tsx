...
export function DriverStatePanel({ lap }: { lap: LapSnapshot }) {
  ...
  return (
    <section className="panel driver-panel">
      ...
      <div className="state-meta"><div><span>CONFIDENCE</span><strong>{lap.confidence}%</strong></div><div><span>3-LAP SHIFT</span><strong className="text-orange">{lap.trend === "RISING" ? "RISING" : lap.trend === "FALLING" ? "FALLING" : "STABLE"}</strong></div></div>
